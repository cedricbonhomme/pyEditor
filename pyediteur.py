...
class editeur:
	def __init__(self):
		...
		self.images =('./images/filenew','./images/closefile','./images/fileopen','./images/filesave','./images/editcut','./images/editcopy','./images/editpaste','./images/undo','./images/redo','./images/searchfind')
		self.textes =('Nouveau','Fermer,''Ouvrir','Sauver','Couper','Copier','Coller','Annuler','Refaire','Chercher')

		self.createmenubar()
		self.createtoolsbar()

		# ---- D�finitions des formats de fichiers ----
		self.formats = [
		('Format', '*.txt'),
		('Tous', '*.*')
		]
	...
	def createmenubar(self):
		...
		self.filemenu.add_command(label="Enregister sous...", command = self.sauver)
		self.filemenu.add_separator()
	...
	def createtoolsbar(self):
		...
		for b in range(nBou):
			...
			bou = Button(self.toolsbar, image =self.photoI[b], relief =GROOVE, command = self.action(b))
	...
	def sauver(self):
		file = tkFileDialog.asksaveasfile(parent = self.root, filetypes = self.formats, title ="Enregistrer le fichier", mode='w')
		if file != None:
			texte = self.root.text.get(1.0, END)
			try:
				file.write('%s' % texte)
	...
	def ouvrir(self):
		file = tkFileDialog.askopenfile(parent = self.root, filetypes = self.formats, mode='rb', title='Ouvrir un document')
		if file != None:
			try:
				data = file.read()
				file.close()
			...
			print "Octets lu du fichier %s : %d octets." % (filename, len(data))
